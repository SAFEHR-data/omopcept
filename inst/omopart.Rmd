...
neop_snomed_r1s  <- omop_relations_recursive(neop_snomed$concept_id, num_recurse = 1,
                                             r_ids="Subsumes")
...
omop_graph(relations,
           graphtitle="SNOMED hierarchies", titletxtsize = 10,
           legendshow=FALSE, 
...
dlbcls <- omop_names("Diffuse large B-cell lymphoma", exact=TRUE, v_ids = ("SNOMED"))

#going up hierarchy with just 'Is a' creates manageable results
#if you include "Subsumes" you can get thousands
dlbclr7 <- omop_relations_recursive(dlbcls$concept_id, 
                                    r_ids=c('Is a'), 
                                    num_recurse=7) 

#Good going up from DLBCL to Neoplastic disease, only a few concepts, but doesn't look great on slide
dlbclr7 |> omop_graph(ggrlayout = "tree",
                      graphtitle="Up a hierarchy, tree layout",titletxtsize = 10,
                      #saveplot=FALSE, graphtitle=NULL, 
                      palettebrewer = "Set1",
                      legendshow=FALSE, 
                      textcolourvar="recurse_level", nodecolourvar = "recurse_level",
                      width=960, height=1080, units="px", #1920
             nodetxtsize=1, #default=9
...
#Multiple Myeloma Subsumes 11 Is a 1
mms <- omop_names("multiple myeloma",exact=TRUE,d="Condition",v="SNOMED",s="S")
#1
mmsr1 <- omop_relations(mms$concept_id)
#mmsr1 <- omop_relations(mms$concept_id, r_ids=c('Is a','Subsumes'), v="SNOMED")
#returning 14
mmsr1 |> count(relationship_id, sort=TRUE)


mmappedfrom <- mmsr1 |> 
  #filter(vocabulary_id %in% c("SNOMED")) |> 
  filter(relationship_id %in% c('Mapped from'))

mmappedfrom |> count(vocabulary_id, sort=TRUE)
# 1 ICDO3             5
# 2 Read              5
# 3 OXMIS             4
# 4 SNOMED            4
...
#This will be good with some tweaking
#and I could make a function to do this for any concept
#(advantage for display that it is quicker than recursive ones) 
mmappedfrom |>
  omop_graph(graphtitle="Multiple myeloma mapped from",titletxtsize = 10, 
             palettebrewer = "Set1",
             legendshow=TRUE, legendtxtsize=6, legendcm=0.5,
             textcolourvar="vocabulary_id", nodecolourvar="vocabulary_id",
             width=960, height=1080, units="px", #1920
             nodetxtsize=1.2, #default=9
             edgewidth=0.5,#default=0.1
