---
title: "Getting drug classes from the hierarchy"
...
**DRAFT** much of this has been replaced by `omop_drug_lookup_create()`    


## Use case

Researchers want to be able to identify drug classes from an omop extract.
Maybe 
1. antibiotics
1. broad spectrum antibiotics
1. specific antibiotic classes

A need to map blood count components from two different source systems to OMOP. There are different potential OMOP concept ids. How to choose between them ? 

Will help deal with different institutions having different OMOP concept ids for essentially the same drugs.
...
```{r CodeListGenerator getATCCodes}
...
Potential `omopcept` workflow :

1. use `omopcept::omop_names()` to look at what ATC classifier concepts there are.
1. 
T
